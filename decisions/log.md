...
[2026-06-29] DECISION: Pre-prod security hardening pass | REASONING: legacy plaintext passwords + missing headers must be mitigated before go-live | CONTEXT: must_change_password column (migrated users flagged true via ETL + bulk UPDATE of existing 972); ForcePasswordChange middleware pins them to /password/change (current-pw required, min8, different); SecurityHeaders middleware (XFO/XCTO/Referrer/Permissions) on web group; app grep-clean of secrets. Ops checklist in context/security.md (ROTATE leaked email pw in legacy config.php, prod APP_DEBUG=false, HSTS, login throttle). HardeningTest 5 tests; full suite 35 pass / 104 assertions.

[2026-06-29] DECISION: Phase 4b lawyer application workflow complete | REASONING: daftar/tarik-diri approval on butiran_peguam_panel_2 with role separation | CONTEXT: PermohonanPeguamController index/show/sokong(pengarah)/keputusan(admin|koordinator)/tarikDiri. Approve PROMOTES applicant into peguam_panel (idempotent by kpBaru) so they appear in agihan. Status codes 0=baharu/1=lulus/2=tolak/3=tarik. Phase4bTest 7 tests; full suite 30 pass / 88 assertions.
