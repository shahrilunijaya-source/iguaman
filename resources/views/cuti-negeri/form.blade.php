...
@section('title', $mode === 'create' ? 'Daftar Cuti Negeri' : 'Kemaskini Cuti Negeri')

@php
    $isCreate = $mode === 'create';
    $action = $isCreate ? route('cuti-negeri.store') : route('cuti-negeri.update', $cuti);
...
@section('content')
    <div class="tap-head">
        <div>
            <h1 class="tap-head__title">{{ $isCreate ? 'Daftar Cuti Negeri' : 'Kemaskini Cuti Negeri' }}<span class="dot"></span></h1>
            <p class="tap-head__sub">{{ $isCreate ? 'Daftar cuti khusus negeri.' : $cuti->nama_cuti }}</p>
        </div>
        <div class="tap-head__cluster">
            <a href="{{ route('cuti-negeri.index') }}" class="tap-head__btn">Batal</a>
        </div>
    </div>
    ...
    <form method="POST" action="{{ $action }}">
        ...
        <div style="display:flex; gap:10px; justify-content:flex-end; align-items:center;">
            <a href="{{ route('cuti-negeri.index') }}" class="btn btn--ghost">Batal</a>
            <button type="submit" class="btn btn--primary">{{ $isCreate ? 'Daftar' : 'Simpan' }}</button>
        </div>
    </form>

    @unless ($isCreate)
        <form method="POST" action="{{ route('cuti-negeri.destroy', $cuti) }}" onsubmit="return confirm('Padam cuti negeri ini?')" style="margin-top:14px;">
            @csrf @method('DELETE')
            <button type="submit" class="btn btn--ghost" style="color:var(--danger);">Padam Cuti Negeri</button>
        </form>
    @endunless
