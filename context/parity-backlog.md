...
**REMAINING:**
- 🟡 EPIC G — Cuti Umum CRUD **DONE** (`CutiController` + `/cuti` + `CutiNegeri` 16-slot `idnegeri` + `selenggara.cuti` perm, 11 tests). Lebih Masa 7-day auto-reassign **DONE** (`LebihMasaService` + `agihan:lebih-masa` command + daily scheduler + `KesLebihMasaMail`; offer status '1' >7 days → forms '4', sejarah '7' marker + `permohonan_kali`++, branch Pengarah notified; 3 tests, isolated from concurrent agihan refactor). **Still remaining:** agihan transition emails (PPUU / Ketua Cawangan / Pengarah Negeri / Pengarah / PP on the EPIC A 3-tier transitions) — deferred until batch-7-rbac settles.
